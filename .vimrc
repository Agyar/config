...
" OLD STUFF
  ...
  "augroup filetype
    " au BufRead,BufNewFile *.jflex,*.flex set filetype=jflex
    " au BufRead,BufNewFile *.cup set filetype=cup
  "augroup END
  "au Syntax jflex so ~/.vim/syntax/jflex.vim
  "au Syntax cup so ~/.vim/syntax/cup.vim
  " Hilight every occurence of the word under cursor 
  " autocmd CursorMoved * exe printf('match TabLine /\V\<%s\>/', escape(expand('<cword>'), '/\'))

  "set statusline=%t[%{strlen(&fenc)?&fenc:'none'},%{&ff}]%h%m%r%y%=%c,%l/%L\ %P

  "au BufNewFile,BufRead,BufEnter *.cpp,*.cxx,*.hxx,*.h,*.hpp set omnifunc=omni#cpp#complete#Main "see http://vim.wikia.com/wiki/VimTip1608 for more tips on c++ completion
